    ...
    with open(report_file, "w", encoding="utf-8") as f:
        ...
        if not added and not removed:
            ...
            for acc in added:
                info = new_data[acc]
                f.write(
                    f"  [+] {acc} ({info.get('organism_name', 'Unknown')} | taxid: {info.get('taxid', 'Unknown')})\n"
                )
...
def main():
    ...
    # 加载新数据
    print("解析新下载的数据...")
    added_accessions, removed_accessions, data_old, data_new = parse_summary_file(
        PREVIOUS_FILE, temp_file
    )
